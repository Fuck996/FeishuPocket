...
**版本：** v1.5.1 | **更新时间：** 2026-04-27 | **内容：** 完成功能或修复后默认推送 GitHub 并触发 feishu-pocket 镜像构建
...
#### 4. 代码推送
- **默认要求**：只要本轮完成的是功能开发、BUG 修复、UI 重构、配置能力增强等可交付改动，AI 在完成本地验证后应默认执行 Git 提交与推送，不再等待用户额外说明“提交”或“上传 GitHub”。
- **Git 提交**：
  ```bash
  git add -A
...
- **仅以下情况允许不推送**：
  - 用户明确要求“先不要提交/不要推送”
  - 当前环境缺少 GitHub 鉴权或远端推送权限
  - 本地仍有与当前任务冲突、且无法安全合并的未解决改动

#### 5. 镜像构建
- **触发原则**：只要本轮完成的是可交付的功能更新或 BUG 修复，并且代码已经成功推送 GitHub，AI 应默认继续触发镜像构建；不再等待用户额外提出“构建镜像 / 发布镜像 / 构建发布”。
- **构建来源**：默认使用**刚推送到当前分支的最新代码**触发 `workflow_dispatch`；仅当用户明确要求 Tag / Release 时，才切换为对应发布流程
- **镜像名称**：统一使用 `feishu-pocket`
- **构建方式**：优先由 AI 直接触发 GitHub Actions 工作流并推动镜像构建；仅在缺少鉴权能力、仓库权限或外部网络受限时，才退回为提示用户手动操作
- **执行顺序**：本地验证通过 → Git 提交 → Git 推送 → 触发镜像构建 → 最后汇报结果
- **参数说明**：
  - `image_tag`：镜像标签（如 `v1.3.3`）
  - `build_title`：构建标题
  - `build_description`：构建描述
  - `push_to_registry`：是否推送到镜像仓库
- **默认标签规则**：若无用户额外指定，默认使用当前版本号作为 `image_tag`，并与 `frontend/package.json`、`backend/package.json`、`/api/version` 保持一致
