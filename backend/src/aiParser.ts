import { ParsedBotAction } from './types.js';
...
function normalizeAction(action: ParsedBotAction): ParsedBotAction {
  if (
    action.intent === 'set_daily_allowance'
...
function tryParseBalanceAdjust(message: string): ParsedBotAction | null {
...
export async function parseBotAction(message: string, openAiApiKey?: string, openAiBaseUrl?: string, model?: string): Promise<ParsedBotAction> {
  if (!openAiApiKey) {
  ...
  消息: ${message}`;
    ...
    const parsed = normalizeAction(JSON.parse(jsonText) as ParsedBotAction);
    if (!parsed.intent) {
      return parseByRegex(message);
    }
