...
import { buildCardSchema, sendFeishuCard, FeishuCardPayload } from './feishu.js';
import { SchedulerService } from './scheduler.js';
import { JsonStore } from './store.js';
import { checkAllModelBalances } from './modelBalance.js';
import { ChildProfile, MoneyTransaction, ParsedBotAction, UserAccount, UserRole, WeeklySummary, ModelConfig, ModelProvider, PromptTemplate, RobotConfig, BotLogEntry } from './types.js';
...
async function getFeishuTenantAccessToken(robot: RobotConfig): Promise<string | undefined> {
  ...
  try {
    ...
    const tokenData = await tokenResp.json() as { code?: number; msg?: string; tenant_access_token?: string };
...
async function queryFeishuUserNameByBasicBatch(tenantAccessToken: string, identity: FeishuUserIdentity): Promise<string | undefined> {
  ...
  try {
    for (const candidate of candidates) {
      ...
      if (!resp.ok) {
        try {
          const errData = JSON.parse(rawText) as { code?: number; msg?: string };
...
async function uploadFeishuImageByUrl(tenantAccessToken: string, imageUrl: string): Promise<string | undefined> {
  try {
    ...
    const uploadData = await uploadResp.json() as { code?: number; msg?: string; data?: { image_key?: string } };
...
function buildHelpCardPayload(): FeishuCardPayload {
  return {
    title: '快捷指令说明',
    lines: [
...
function getRelativeWeekRange(offsetWeeks: number): { start: Date; end: Date; labelStart: string; labelEnd: string } {
  const now = new Date();
...
function getRelativeMonthRange(offsetMonths: number): { start: Date; end: Date; labelStart: string; labelEnd: string } {
  const now = new Date();
...
function calculateRemainingAmountAt(childId: string, periodEnd: Date): number {
...
function summarizeChildPeriod(child: ChildProfile, start: Date, end: Date): {
...
function formatPeriodAmount(value: number, hasData: boolean): string {
  return hasData ? amountYuan(value) : '无数据';
}

function formatComparisonHighlight(currentRemaining: number, previousRemaining: number, compareLabel: string): string {
...
async function buildWeeklyStatsCardPayload(child: ChildProfile): Promise<FeishuCardPayload> {
...
async function buildMonthlyStatsCardPayload(child: ChildProfile): Promise<FeishuCardPayload> {
...
function buildControlCenterCardPayload(child: ChildProfile): FeishuCardPayload {
  return {
    title: '控制中心',
...
async function sendRobotCardAndLog(robot: RobotConfig, payload: FeishuCardPayload, chatId?: string, childId?: string): Promise<string | undefined> {
...
async function triggerRobotQuickAction(robot: RobotConfig, actionKey: string, chatId: string | undefined, actor?: {
  openId?: string;
  displayName?: string;
...
type MenuBridgeActionKey = typeof MENU_BRIDGE_ALLOWED_ACTIONS[number];

function normalizeMenuBridgeActionKeys(input: unknown): MenuBridgeActionKey[] {
  const source = Array.isArray(input) ? input : MENU_BRIDGE_ALLOWED_ACTIONS;
  const normalized = Array.from(new Set(
    source
      .map((item) => String(item ?? '').trim())
      .filter((item): item is MenuBridgeActionKey => MENU_BRIDGE_ALLOWED_ACTIONS.includes(item as MenuBridgeActionKey))
  ));
...
function buildMenuBridgeUrl(baseUrl: string | undefined, pathWithQuery: string): string {
...
function tryResolveBaseUrlFromWebhookUrl(webhookUrl?: string): string | undefined {
...
function resolveMenuBridgeBaseUrl(robot: RobotConfig, incomingBaseUrl?: string): string | undefined {
...
function ensureRobotMenuBridgeToken(robotId: string, chatId: string): string {
  let token = '';
  store.update((draft) => {
...
function collectRobotMenuChatIds(robot: RobotConfig): string[] {
...
async function syncRobotGroupMenuToChat(robot: RobotConfig, chatId: string): Promise<void> {
  ...
  try {
    result = rawText ? JSON.parse(rawText) as { code?: number; msg?: string } : {};
  } catch {
    result = {};
  }
...
async function syncAllRobotGroupMenusOnStartup(): Promise<void> {
...
function resolveMatchedRobots(senderOpenId: string): RobotConfig[] {
...
let wsClientAppId: string | undefined;
let wsClientLastReadyAt: string | undefined;
let wsClientLastError: string | undefined;
let wsClientLastReconnectingAt: string | undefined;
let wsClientLastReconnectedAt: string | undefined;
let wsClient: Lark.WSClient | undefined;

async function initFeishuWsClient(): Promise<void> {
  // 仅使用启用且 app 模式的机器人建立 WS 长连接
  const robot = store.getSnapshot().robots.find((r) =>
    r.enabled && r.feishuMode !== 'webhook' && r.feishuAppId && r.feishuAppSecret
  );
  ...
  try {
    ...
    const eventDispatcher = new Lark.EventDispatcher({}).register({
      ...
      'card.action.trigger': async (data: any) => {
        try {
          return await handleCardAction(data as unknown);
        } catch (error) {
...
app.post('/api/robots', requireAuth, requireRole('admin'), (req, res) => {
  const {
    name, enabled, childIds, controllerOpenIds,
    feishuMode, feishuAppId, feishuAppSecret, feishuWebhookUrl,
    feishuVerificationToken, feishuSigningSecret, feishuDefaultChatId, menuBridgeBaseUrl
  } = req.body as {
    name?: string;
    enabled?: boolean;
    childIds?: unknown;
...
app.put('/api/robots/:robotId', requireAuth, requireRole('admin'), (req, res) => {
  ...
  store.update((draft) => {
    ...
    if (menuBridgeBaseUrl !== undefined) robot.menuBridgeBaseUrl = String(menuBridgeBaseUrl).trim().replace(/\/+$/, '');
...
app.post('/api/robots/:robotId/actions/:actionKey', requireAuth, requireRole('admin'), async (req: AuthedRequest, res) => {
...
app.get('/api/robots/:robotId/chats', requireAuth, requireRole('admin'), async (req, res) => {
  ...
  try {
    ...
    const allItems: Array<{ chat_id: string; name: string }> = [];
...
app.delete('/api/config/reward-rule', requireAuth, requireRole('admin'), async (req, res) => {
  const { childId, keyword } = req.body as { childId?: string; keyword?: string };
  if (!childId || !keyword) {
...
async function handleCardAction(eventData: unknown): Promise<Record<string, unknown>> {
  ...
  const contextObj = data.context as Record<string, unknown> | undefined;
  const chatId = contextObj?.open_chat_id as string | undefined;
  const robot = resolveDiagnosticRobot(chatId);
  ...
  switch (actionType) {
    ...
    case 'undo_transaction': {
      const transactionId = actionValue['transactionId'] as string | undefined;
      const childId = actionValue['childId'] as string | undefined;
  ...
  try {
    switch (action.intent) {
      ...
      case 'reward_from_message': {
        if (!targetChild || action.amount === undefined) throw new Error('缺少奖励金额');
        await applyTransaction({
          childId: targetChild.id,
          amount: Math.abs(action.amount),
...
app.post(['/api/feishu/webhook', '/api/feishu/events'], async (req: AuthedRequest, res) => {
  ...
  if (eventType === 'card.action.trigger') {
    ...
    const eventData = (bodyData.event as Record<string, unknown> | undefined) ?? bodyData;
    try {
