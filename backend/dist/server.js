...
function normalizeFeishuTimestampMs(raw) {
...
function extractFeishuEvent(reqBody) {
    ...
    const messageCreateTimeMs = normalizeFeishuTimestampMs(message.create_time)
...
async function sendRobotCard(payload, chatIdOverride, childId) {
    ...
    const finalPayload = hasBalanceLine
        ? payload
        : {
            ...payload,
...
function buildModelBalanceLine() {
...
async function uploadFeishuImageByDataUrl(tenantAccessToken, dataUrl) {
    try {
...
async function ensureChildFeishuAvatarKey(child) {
    ...
    const childBoundRobot = store.getSnapshot().robots.find((r) => r.enabled
        && r.feishuMode !== 'webhook'
        && r.feishuAppId
        && r.feishuAppSecret
        && r.childIds.includes(child.id));
    const robot = childBoundRobot ?? store.getSnapshot().robots.find((r) => r.enabled
        && r.feishuMode !== 'webhook'
        && r.feishuAppId
        && r.feishuAppSecret);
    if (!robot) {
...
function resolveActorDisplay(source, actorUserId, actorDisplayName) {
    const sourceLabels = {
        admin: '管理员',
        operator: '操作员',
        bot: '飞书用户',
...
async function processBotMessage(channel, senderOpenId, senderUserId, senderUnionId, senderDisplayNameHint, messageCreateTimeMs, senderType, messageType, contentRaw, chatId, messageId) {
    ...
    const matchedRobots = resolveMatchedRobots(senderOpenId ?? '').filter((robot) => channel === 'ws' ? robot.feishuMode !== 'webhook' : robot.feishuMode === 'webhook');
